...
public abstract class WildFlyElytronBaseProvider extends VersionedProvider {
    ...
    static void putMakedPasswordImplementations(Consumer<Service> consumer, Provider provider) {
        for (String algorithm : MASKED_ALGORITHMS) {
            consumer.accept(new ProviderService(provider, PASSWORD_FACTORY_TYPE, algorithm, "org.wildfly.security.password.impl.PasswordFactorySpiImpl", emptyList, emptyMap, false, false));
        }
    }
    ...
    static void putMakedAlgorithmParametersImplementations(Consumer<Service> consumer, Provider provider) {
        for (String algorithm : MASKED_ALGORITHMS) {
            consumer.accept(new ProviderService(provider, ALG_PARAMS_TYPE, algorithm, "org.wildfly.security.password.impl.MaskedPasswordAlgorithmParametersSpiImpl", emptyList, emptyMap, false, false));
        }
    }
